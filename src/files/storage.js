...
export function listStoredFiles(filesDir, chunksDir, scope, ownerId, ownerUsername) { mkdirSync(filesDir, { recursive: true }); mkdirSync(chunksDir, { recursive: true }); const descriptions = readIndex(join(filesDir, "files_index.json")); const conflicts=pruneOrphanedConflicts(filesDir); return readdirSync(filesDir).filter((name) => name.toLowerCase().endsWith(".txt")).sort().map((name) => { const stem = parse(name).name; const chunkPath = join(chunksDir, `${stem}.json`); const data = readIndex(chunkPath); return { name, stem, scope, owner_id: ownerId, owner_username: ownerUsername, indexed: existsSync(chunkPath), status: existsSync(chunkPath) ? "indexed" : "indexing", chunks: Number(data.total ?? 0), description: descriptions[stem] ?? "", inconsistencies: conflictsResponse(conflicts[stem],existsSync(chunkPath)?"unchecked":"unindexed"), security: { has_any: false, risk: "none", matches: [], status: existsSync(chunkPath) ? "unchecked" : "unindexed" } }; }); }
