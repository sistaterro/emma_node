...
export function listStoredFiles(filesDir, chunksDir, scope, ownerId, ownerUsername) { mkdirSync(filesDir, { recursive: true }); mkdirSync(chunksDir, { recursive: true }); const descriptions = readIndex(join(filesDir, "files_index.json")); return readdirSync(filesDir).filter((name) => name.toLowerCase().endsWith(".txt")).sort().map((name) => { const stem = parse(name).name; const chunkPath = join(chunksDir, `${stem}.json`); const data = readIndex(chunkPath); return { name, stem, scope, owner_id: ownerId, owner_username: ownerUsername, indexed: existsSync(chunkPath), status: existsSync(chunkPath) ? "indexed" : "indexing", chunks: Number(data.total ?? 0), description: descriptions[stem] ?? "", inconsistencies: { has_any: false, matches: [], status: existsSync(chunkPath) ? "unchecked" : "unindexed" }, security: { has_any: false, risk: "none", matches: [], status: existsSync(chunkPath) ? "unchecked" : "unindexed" } }; }); }
/** @param {string} filesDir @param {string} chunksDir @param {string} stem */
export function deleteStoredFile(filesDir, chunksDir, stem) { stem = sanitizeStem(stem); const deleted = []; for (const path of [join(filesDir, `${stem}.txt`), join(chunksDir, `${stem}.json`)]) if (existsSync(path)) { rmSync(path); deleted.push(basename(path)); } for (const name of ["files_index.json", "conflicts_index.json", "security_index.json"]) pruneIndex(join(filesDir, name), stem); if (!deleted.length) throw new HttpError(404, `File '${stem}' not found`); return deleted; }
